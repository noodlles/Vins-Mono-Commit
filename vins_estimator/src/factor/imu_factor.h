...
//delta_q = Qi.inverse() * Qj;
        ...
        // 因此需要把P^-1做LLT分解， A=LL^T ===> P^{-1}=LL^T
        // sqrt_info= L^T
        // 那么 residual = (L^T r)
        // 最后有: d=(L^T r)^T (L^T r) = r^T P^-1 r
        ...
        if (jacobians)
        {
            // 获取预积分的[误差递推函数]中pqv关于ba、bg的Jacobian
            ///这里的东西，是用来根据 泰勒一阶近似来更新 预积分量用到的，
            ///<注意>： 不是预积分误差 关于 ba、bg的雅克比，而是对 delta ba , delta bg 的雅克比
            /// (也就是对预积分误差关于 delta ba ,delta bg的线性化)
...
#else
                // 由于优化过程中， 第i帧的状态bgi会不断调整， 那么对应的预积分量也要进行更新
                // 由于下面需要用到q{bi<--bj} ， 这里只更新 姿态预积分量q{bi<--bj} 即可
